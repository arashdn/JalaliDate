<?php
/* OOP class for date By Arash Dargahi , http://arashdn.com
 * Based on jdf.php v2.55 by http://jdf.scr.ir
 */
class JalaliDate
{
    ...
    function __construct($timeStamp = null, $timeZone = null)
    {
        if($timeStamp == null || $timeStamp=='')
            $this->timeStamp = time();
        else
    ...
    public function format($fstr , $trNum = 'fa')
    {
        return $this->jdate($fstr, $this->timeStamp, '', $this->timeZone, $trNum);
    }
    
    //Begin jdf.php v2.55 ,by http://jdf.scr.ir
    ...
    {
        $dist = time()-$this->getTimeStamp();

        if($dist<86400)//86400 : 24hour
            return $this->format('H:i');
        else if($dist<31536000)//31536000:one year
            return $this->format('j F').'<br />'.$this->format('H:i');
        else
            return $this->format('j F Y').'<br />'.$this->format('H:i');
    }
